...
if  __name__ == "__main__":
    ...
    save_file_name = cross_str + {'letter': learning_algorithm + '_char',
                                  'ozone': learning_algorithm + '_ozone',
                                  'cancer':learning_algorithm + '_cancer'}[current_dataset]
    ...
    parameters = {'init': ['k-means++','random'],
                  'n_clusters': range(1,len(le.classes_)+1),
                  'n_init': [10]}
    ...
    for init_parameter in parameters['init']:
        for n_cluster in parameters['n_clusters']:
            bench_k_means(KMeans(init=init_parameter
                                 ,n_clusters=n_cluster),
                          init_parameter+' '+str(n_cluster),
                          X,
                          y)
